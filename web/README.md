...
public/
  ...
  login.html      Password login for the device monitor
  dashboard.html  Protected page listing all FlugVel devices
  styles.css      Shared styling for all pages

functions/
  api/login.js         POST — checks the admin password, issues a JWT cookie
  api/logout.js        POST — clears the session cookie
  api/me.js            GET  — "am I logged in?" check used by dashboard.html
...
the database or in code). Logging in on `login.html` posts the password to
